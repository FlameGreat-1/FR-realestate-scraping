    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            if not listing_urls and seed_urls:
                print(f"  -> [Playwright] No direct listing URLs for {base_url}; trying seed crawl...")
                visited = set()
                queue = list(seed_urls)
                rescued_urls = []
                while queue and len(visited) < 40 and len(rescued_urls) < 20:
                    current = queue.pop(0)
                    if current in visited:
                        continue
                    visited.add(current)
                    page_html = await safe_fetch_html(page, current, timeout=15000)
                    if not page_html:
                        continue
                    listing = await extract_listing_data(current, page_html, domain_info)
                    if listing.price and any([listing.reference_id, listing.property_type, listing.location, listing.surface_area, listing.rooms, listing.bedrooms]):
                        rescued_urls.append(current)
                        all_listings.append(listing)
                        continue
                    for child in set(re.findall(r'href=[\'\"]([^\'\"]+)[\'\"]', page_html, re.I)):
                        if child.startswith("/"):
                            child = urljoin(current, child)
                        if child.startswith(("http://", "https://")) and child not in visited and child not in queue:
                            queue.append(child)
                if rescued_urls:
                    listing_urls = []
